...
describe('flow-core/scorer', () => {
  ...
  describe('buildIssueFamilies — grouping', () => {
    ...
    it('sorts families by descending severity, then title ascending', () => {
      ...
      // two 'high' families: dml_inside_loops sorts before hard_coded_ids on title
  ...
  describe('scoring formula — per-severity behaviour', () => {
    // Validates the documented v1.2.2 formula:
    //   deduction = min( appearance + weight * log2(instanceCount + 1), cap )
  ...
  describe('calculateScore — totals and floors', () => {
    ...
    it('floors at 0 when deductions exceed 100', () => {
      // Pile up many capped highs (22 each) — enough to drive score below zero
    ...
    it('lands an Account-Verification-shaped flow in the 40–55 band', () => {
      ...
      const findings: Finding[] = [
        ...
        ...Array.from({ length: 8 }, () => f('outdated_api_version', 'medium')), // ~9.5
      ];
      const families = buildIssueFamilies(findings);
      const result = calculateScore(families);
  ...
  describe('counts', () => {
    it('counts families per severity (not findings)', () => {
      const families = buildIssueFamilies([
        f('a', 'high'),
        f('a', 'high'), // same family — should not double-count
        f('b', 'medium'),
        f('c', 'low'),
        f('d', 'low'),
      ]);
