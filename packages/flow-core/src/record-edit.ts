// Pure editability model for record view / edit / clone.
//
// Shared by the CLI (`sfdt record`), the MCP tools, and the Chrome extension's
// inspect-record — one model, so every surface refuses the same field for the
// same stated reason.
//
...
// Structural types, declared here rather than imported.
//
// This module lives in flow-core so the CLI, MCP and the browser extension all
// decide "is this field writable" from ONE implementation — a formula field has
// to be refused identically everywhere, and two copies of that rule is two
// answers waiting to diverge. flow-core takes no dependency on either consumer,
// so the shapes it needs are declared structurally: the extension's richer
// FieldDescribe (describe-cache.ts) and its SalesforceRestErrorDetail both
// satisfy these, and the CLI can build them straight from a describe payload.
//
// Every permit/deny flag is optional for the same reason it is optional in the
// extension's own declaration: a describe payload always sends booleans, but a
// hand-built fixture need not, and consumers must therefore read each one as
// `=== true`. For PERMIT flags (updateable, createable) that fails CLOSED — an
// absent value denies the write. For DENY flags (calculated, autoNumber,
// htmlFormatted, encrypted) it fails OPEN, meaning "not excluded".

/** The subset of an sObject field describe this model reads. */
export interface FieldDescribe {
  ...
  /** PERMIT flags — read as `=== true`, so absence denies. */
  updateable?: boolean;
  createable?: boolean;
  /** DENY flags — read as `=== true`, so absence means "not excluded". */
  calculated?: boolean;
  autoNumber?: boolean;
...
/** One record from a Salesforce REST error body: `[{message, errorCode, fields}]`. */
