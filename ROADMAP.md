...
- **GUI SOQL query console** — "SOQL Console" dashboard page surfacing the `sfdt soql` family (schema browse/describe, query validation, plans, bounded execution with JSON/CSV export) via thin `/api/soql/*` routes over the existing `soql-runner.js` — no logic reimplementation, the runner stays the single engine. Split out of the initial `sfdt soql` delivery by decision 2026-07-29 (backlog item D-4) — **In develop**
