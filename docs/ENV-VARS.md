...
| `SFDT_MCP_PROJECT_ROOTS` | Optional allowlist pinning which projects an MCP server will serve, colon-separated absolute paths (`path.delimiter`). **Since 0.26.0 the default is set by how the server was launched:** one started *inside* a project serves that project only; a *neutral* server (started outside any project) routes freely, which is its purpose. This variable widens a project-bound server to a named set. Every MCP tool accepts a `projectRoot`, and that value is chosen by the *model* — which this CLI feeds untrusted org content — so a call the operator reads as "the current project" can name a different checkout and run against that org. Cross-project routing is a real feature (one server, several checkouts), so this is opt-in rather than default-deny. Set it on any machine holding more than one customer's checkout. See [ARCHITECTURE §18](./ARCHITECTURE.md#18-threat-boundaries) |
