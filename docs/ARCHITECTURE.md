...
**Three local stores, three different jobs.** `logs/history.db` (`run-history.js`) indexes RUNS —
counts only, pruned to 200 per type. `logs/audit.json` (`audit-logger.js`) is the GUI's action log —
a JSON array, rewritten whole, capped at 1000. `logs/ledger.jsonl` (`ledger.js`) is the change
ledger: append-only, hash-chained, and the only one that records the STATE a change replaced. The
first two are not append-only — one deletes rows, the other rewrites the file — which is exactly why
the ledger is a third store rather than a column on either. It is also the one carve-out to golden
principle #5: a ledger write that fails aborts the org write it was about to record.
...
| `.sfdt/config.json` | **Untrusted input.** `sfdt init` gitignores only `*.local.json`, so `config.json` is committed and arrives with whatever repo was cloned. `loadConfig()` refuses settings **by capability, not by key name** — the first version enumerated five keys, so every new path-, URL- or privilege-shaped key shipped unguarded until someone remembered it. The classes: **code execution** (`plugins[]`, `pluginOptions.autoDiscover`), **process spawn** (`mcp.salesforce.command`/`args`), **privilege** (`ai.agent.enabled`/`allowWrite`), **destination** (non-loopback `ai.baseURL`; a channel's literal `webhookUrl`/`url`, `*Env` forms still allowed), **path** (every key in `PROJECT_PATH_CONFIG_KEYS`, contained to the project root by `isPathWithinRoot`). All overridable by `SFDT_ALLOW_UNSAFE_CONFIG=1`. Note `autoDiscover` is guarded for the same reason as `plugins[]`, not as a lesser sibling: it imports every `sfdt-plugin-*` in the project's `node_modules/` **and** every file in `.sfdt/plugins/`, so guarding only `plugins[]` would leave two equivalent paths open. The opt-in is an env var **by necessity**: an in-config flag would be set by the same attacker. See `src/lib/config-trust.js` |
| `.sfdt/prompts.json` | **Untrusted input, same reasoning.** It is not inert data — it is the instruction text handed to an AI provider, including the write-capable auto-fix loop, and it ships with the clone. `getPrompt()` ignores repo-supplied overrides unless `SFDT_ALLOW_UNSAFE_CONFIG=1` and says once what it ignored. Free-form instruction text has no safe subset, so the choice is the whole file or none of it; the dashboard's prompt editor reads and writes the raw file either way, since filtering the read side would make a save drop the user's other overrides. `sfdt init` recommends gitignoring it |
...
| AI subprocesses | CLI providers run in read-only sandboxes; the write-capable agent loop is granted **only** by `SFDT_ALLOW_AI_WRITE=1` in the operator's environment and is dry-run-verified per turn. It used to be gated on `ai.agent.enabled` + `ai.agent.allowWrite`, both read from the committed config file — two booleans an attacker sets in one edit are one gate, not two, and "double-opt-in" was never true of them. The variable is separate from `SFDT_ALLOW_UNSAFE_CONFIG` on purpose: trusting a repo's plugin list should not imply handing a model `Edit`. The sandbox is gated on `allowedTools` **existing**, not on it being non-empty — `[]` means "no tools", the most restrictive request, and must not read as "unrestricted" |
