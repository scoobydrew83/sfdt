...
if [[ -z "$ENFORCE_TESTS" ]]; then
    ...
    if [[ -z "$DEFAULT_ORG" ]]; then
        ...
        # Key discriminator: if .result.summary.outcome is present, tests actually ran
        # (regardless of exit code — sf v2 exits 100 on test failures, not just 1)
        ...
        if [[ -n "$test_status" ]]; then
            ...
            if [[ "$test_status" == "Passed" ]]; then
                ...
                # Print individual failing test names and error messages
                echo ""
            ...
            # Command failed before any tests ran — connectivity or org issue
            # jq 'empty' produces no output and exits 0, so use explicit "" fallback instead
        ...
        # ── Check 6: Coverage threshold (only when tests actually ran) ────────────
