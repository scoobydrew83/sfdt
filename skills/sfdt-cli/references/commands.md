...
| `sfdt field impact <Object.Field> [--org] [--links] [--json]` | "What writes this field?" — flows (parsed, not just referenced), workflow field updates, and an Apex text search. Each row is `confirmed` (the metadata states the write) or `inferred` (a lead only). The engine is `@sfdt/flow-core`'s `analyzeFieldImpact`, shared with the Chrome extension, so both surfaces scan to the same depth. **Read the scan-scope notes**: an empty result means no writer was found by three bounded scans, never that none exists |
| `sfdt field usage <Object> [--org] [--population] [--json]` | Sweeps EVERY field on an object, batching the dependency lookup so N fields cost ceil(N/200) queries. Three states: unreferenced, referenced, and **unknown** (a standard field has no CustomField record; a failed batch also lands here) — unknown is never clean. `--population` counts non-null values, and only then can a field be `safeToRemove`: custom AND unreferenced AND zero values AND not required/unique. Every rejection carries a `keepReason` |
