...
describe('extension/lib/spa-router', () => {
  ...
  it('does not fire for non-Salesforce URLs even if location changes', async () => {
    // happy-dom blocks cross-origin history.replaceState, so we simulate a
    // non-Salesforce navigation by injecting a stub window whose location.href
    // we control. The MutationObserver still runs against the real document.
