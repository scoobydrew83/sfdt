...
2. **The SID never leaves the background worker.** All Salesforce REST/Tooling/SOAP calls go through the worker's `sfApiFetch` route (`lib/sf-api-proxy.ts`); `lib/salesforce-api.ts` is a **thin client** that describes a call and parses the reply — it holds no sid and makes no Salesforce fetch. The event-monitor CometD stream also runs in the worker, behind an `sfApiStream` Port (`lib/sf-stream-worker.ts`), so no page code holds the sid there either. Feature/UI/entrypoint code MUST NOT touch `chrome.cookies`, `getSidForUrls`, `getSessionDetails`, or a raw `Authorization: Bearer` header — `test/sid-never-leaves-worker.test.ts` + a scoped ESLint rule enforce this with **zero exceptions** (the guard allowlists nothing; don't add a new exception).
