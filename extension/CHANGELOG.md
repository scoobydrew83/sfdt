...
### Added
- **Debug Logs — auto-refresh + bulk delete (P1-6).** The Debug Logs viewer gains an **Auto-refresh (15s)** toggle in its toolbar — off by default; when on, the log list re-queries every 15 seconds so new logs appear while a trace flag is active. The poll interval is owned at feature scope and cleared on view close and on feature teardown (kill-switch / route change), so no orphan timer survives. A **Delete all logs** button bulk-deletes the loaded ApexLog rows via the Tooling API behind a count-confirm dialog ("Delete N logs?"); the accessible dialog carries `role="dialog"`/`aria-modal`, closes on Esc (capture phase) and backdrop click, traps Tab between its two buttons, and restores focus to the opener — after deletion the list refreshes. The `sid` stays in the worker throughout (existing proxied `api` client). ponytail: delete targets the loaded page (bounded by the viewer's page size, exactly the count the dialog quotes), not a silent org-wide sweep.
