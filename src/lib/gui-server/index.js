...
export function createGuiApp(config, version, port = 7654) {
  ...
  app.get('/api/manifest/discover', apiLimiter, async (req, res) => {
    try {
      ...
      const TYPE_MAP = {
        ApexClass:               { pat: '**/classestriggerspagescomponentslwcauraflowsflowDefinitionsobjectsobjectslayoutsflexipagespermissionsetspermissionsetgroupsprofilesstaticresourcescontentassetscustomMetadatacustomPermissionstabsobjectsemail*.email-meta.xml',                    ext: (f) => path.basename(f, '.email-meta.xml') },
  ...
  app.get('/api/dependencies/preflight', apiLimiter, async (req, res) => {
    try {
      ...
      for (const rec of records) {
        ...
        const srcName = rec.MetadataComponentName;
        if (!refName) continue;
        if (manifestComponents.has(refName)) continue;
